...
import DuzzleLib from "./object/DuzzleLib.tsx";
    ...
    useFrame(({ clock }) => {
      ...
      if (groupRef.current) {
        ...
        //   groupRef.current.rotation.y = elapsedTime * 5;
          groupRef.current.rotation.y = elapsedTime * 1/5;
        } else {
      ...
      () =>
        resource.objects.map((object, index) => (
          <group key={`${object.type}-${index}`}>
            <ObjectRenderer object={object} isDay={isDay} />
          </group>
        )),
      [resource.objects, isDay],
    );

    const resourcePathPosition = useMemo(
